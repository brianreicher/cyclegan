...
class ImageDataset(Dataset):
    def __init__(self, root, transforms_=None, unaligned=False, mode='train') -> None:
        self.transform: transforms.Compose = transforms.Compose(transforms_)
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.unaligned:
            item_B:Image = self.transform(Image.open(self.files_B[random.randint(0, len(self.files_B) - 1)]))
        else:
